...
## range = 0 means disabled. Wire upgrades via set_range() and add_affected_type().
...
func is_active() -> bool:
	return range > 0.0 and not affected_types.is_empty()

## Returns all Node2D instances in affected groups within range of from_position.
func get_attracted_nodes(from_position: Vector2) -> Array[Node2D]:
	...
	for group in affected_types:
		for node in get_tree().get_nodes_in_group(group):
			if node is Node2D and node.global_position.distance_to(from_position) <= range:
